...
namespace Cybtans.Entities.MongoDb.Tests
{
    ...
    public class CustomerCosmoDbTests : IClassFixture<CustomerCosmoDbFixture>
    {
        IObjectRepository<Customer> _customers;
        public CustomerCosmoDbTests(CustomerCosmoDbFixture fixture)
        {
        ...
        {
            var list = await _customers.ListAll(sortBy: x=>x.State);
            Assert.NotEmpty(list);          
            Assert.All(list, x =>
            {
                Assert.NotEmpty(x.Id);
